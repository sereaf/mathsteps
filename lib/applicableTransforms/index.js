...
function normalizeExpressionString(expressionString) {
  ...
  try {
    ...
    throw createError('INVALID_EXPRESSION', 'Не удалось распарсить выражение');
  }
...
function resolvePath(rootNode, path) {
  if (!Array.isArray(path)) {
    throw createError('INVALID_PATH', 'Некорректный selectionPath');
  }
  ...
  for (let i = 0; i < path.length; ) {
    ...
    if (step === 'args') {
      ...
      if (typeof index !== 'number') {
        throw createError('INVALID_PATH', 'Ожидался индекс после args');
      }
      if (!node.args || index < 0 || index >= node.args.length) {
        throw createError('INVALID_PATH', 'Некорректный путь к args');
      }
      parentNode = node;
      ...
      if (!node.content) {
        throw createError('INVALID_PATH', 'Некорректный путь к content');
      }
      parentNode = node;
      parentPath = walked.slice();
      walked.push('content');
      ...
      throw createError('INVALID_PATH', 'Некорректный шаг пути');
    }
  }
...
function listApplicableTransforms(expressionString, selectionPath, options) {
  ...
  if (opts.domain !== 'simplify') {
    throw createError('INVALID_DOMAIN', 'Домен не поддерживается');
  }
...
function applyTransform(expressionString, selectionPath, transformId, options) {
  ...
  if (!match) {
    throw createError('UNKNOWN_TRANSFORM', 'Неизвестный transformId');
  }

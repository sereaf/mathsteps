...
EquationOperations.ensureSymbolInLeftNode = function(equation, symbolName) {
  const hasSymbolOnLeft = containsSymbolNode(equation.leftNode, symbolName);
  const hasSymbolOnRight = containsSymbolNode(equation.rightNode, symbolName);
...
EquationOperations.isolateSymbolOnLeftSide = function(equation, symbolName) {
  ...
  if (Node.Type.isOperator(leftNode)) {
    if (leftNode.op === '+') {
      ...
      // For power operations like x^2 = 4, take nth root of both sides
      // to get x = nthRoot(4, 2)
      // The leftNode is the entire power expression (e.g., x^2)
      // We want to take the nth root where n is the exponent
      inverseOp = 'nthRoot';
...
function performTermOperationOnEquation(equation, op, term, changeType) {
  ...
  // When op is nthRoot, term should be a power node (^) with args[0] = base, args[1] = exponent
  const root = op === 'nthRoot'
        ? (leftTerm.args && leftTerm.args.length > 1 ? leftTerm.args[1] : null)
        : null;
...
function containsSymbolNode (node, symbolName) {
  if (Node.Type.isSymbol(node)) { return node.name === symbolName; }
  else if (Node.Type.isParenthesis(node)) { return (containsSymbolNode(node.content, symbolName)); }
  else if (node.args) { return !!node.args.find((child) => containsSymbolNode(child, symbolName)); }
  else { return false; }
}

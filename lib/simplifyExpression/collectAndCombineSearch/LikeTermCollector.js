...
function getTermName(node, termSubclass, op) {
  ...
  const baseNode = term.getBaseNode()
  // we 'name' terms by their base node name
  // #dent: assume symbol node to avoid costly printAscii function
  let termName = baseNode.name || print.ascii(baseNode);
  // when adding terms, the exponent matters too (e.g. 2x^2 + 5x^3 can't be combined)
  if (op === '+') {
    exponentNode = term.getExponentNode(true)
    const exponent = term.value || term.name || print.ascii(exponentNode);
    termName += '^' + exponent;
  }
  return termName;
}

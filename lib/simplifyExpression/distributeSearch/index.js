...
function distribute(node) {
  if (Node.Type.isUnaryMinus(node)) {
    ...
    return handleExponentNode(node);
  }
  else {
...
// Handles power nodes (base^exponent) with various cases:
// 1) Negative exponents on mult/div/power -> reciprocate
// 2) Addition base -> expand (preserves pedagogical clarity)
// 3) nthRoot base -> expand to repeated multiplication OR convert to exponent
// 4) Multiplication/power base -> distribute exponent
// 5) Division base -> distribute exponent
function handleExponentNode(node) {
  ...
  // Handle negative exponents for mult/power bases only
  // Don't handle division or addition bases with negative exponents
  // as they may need other simplification first
  if (Negative.isNegative(exponent)) {
    if (Node.Type.isOperator(base, '*') || Node.Type.isOperator(base, '^')) {
      return reciprocateTerm(node);
    }
  }
  // Expand addition bases (prioritize for pedagogical clarity)
  else if (Node.Type.isOperator(base, '+')) {
    return expandBase(node);
  }
  // For nthRoot bases, expand to repeated multiplication (existing behavior)
  // This preserves compatibility with existing tests
  else if (Node.Type.isFunction(base, 'nthRoot')) {
    return expandBase(node);
  }
  // Distribute exponent over multiplication or nested powers
  else if (Node.Type.isOperator(base, '*') || Node.Type.isOperator(base, '^')) {
    return distributeExponentOverMultiplicationOrPower(node);
  }
  // Distribute exponent over division
  else if (Node.Type.isOperator(base, '/')) {
...
// Note: Currently not used - we prioritize expandBase for pedagogical clarity
function convertNthRootToExponent(node) {
  ...
  // Check if root and exponent are the same value
  const rootValue = root.value;
  const exponentValue = exponent.value;
  const newExponent = (rootValue && exponentValue && rootValue === exponentValue)
        ? one
...
// Reciprocate term with negative exponent
// e.g. (x*y)^-1 -> 1/(x*y), (2x^2)^-1 -> 1/(2x^2)
...
// Distribute exponent over multiplication or power nodes
...
function distributeExponentOverMultiplicationOrPower(node) {
  ...
  if (Node.Type.isOperator(base, '*')) {
    newTerms = base.args.map(function(term) {
      // Recurse for nested multiplication
      ...
      // Distribute over nested power: (x^2)^2 -> x^(2*2)
      if (Node.Type.isOperator(term, '^')) {
        return distributeOverPowerNode(term, outerExponent);
      }

      // Handle nthRoot terms
...
function distributeOverPowerNode(node, outerExponent) {
  ...
  const newExponent = Node.Creator.parenthesis(
    Node.Creator.operator('*', [exponent, outerExponent]));

  return Node.Creator.operator('^', [base, newExponent]);
}

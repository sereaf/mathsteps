...
const enhancedFunctionsSearch = require('./enhancedFunctionsSearch');
...
function step(node) {
  ...
  const simplificationTreeSearches = [
    ...
    // Enhanced functions: trigonometry, logarithms, advanced power/root operations
    // Applied after basic combinations to avoid interfering with standard simplification
    enhancedFunctionsSearch,
    // e.g. 2 + 2 => 4
    arithmeticSearch,
    // e.g. (2 + x) / 4 => 2/4 + x/4
    ...
    distributeSearch,
    // e.g. abs(-4) => 4, nthRoot simplifications
    functionsSearch,
  ];
